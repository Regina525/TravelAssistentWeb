...
export default function Login() {
 ...
 const handleSubmit = async e =>{
  e.preventDefault();
  try{
 await axios.post("http://localhost:3003/api/users/register",inputs)
 navegate('/login')
    ...
    <>
      ...
      <form className='w-full '>
        <div className="flex h-screen flex-col items-center justify-center w-full">
          ...
          <div className="rounded-md  px-6 shadow-2xl w-1/4">
            ...
            <input name='username' type="text" className='border border-black' onChange={handleChange}/>

            <div className="mb-2 block pt-4">
              <Label htmlFor="email" value="Email" />
            </div>
            <input name='email' type="text" className='border border-black' onChange={handleChange}/>
            
            <div className="mb-2 block pt-4">
              <Label htmlFor="title" value="Palavra-passe" />
            </div>
            <input
              name='password'
              type="password"
            ...
            <div className="w-full">
              <button onClick={handleSubmit} className="mt-10 cursor-pointer rounded-md bg-red px-4 py-2 text-center text-lg font-medium text-white shadow-xl hover:shadow-xl">Cadastrar</button>
            </div>
            {error && <p>Error</p>}
            <div className="my-6">
              <p className="text-md py-4">
                <p>Possui conta? <Link to="/login" className='underline text-blue'>Faca login</Link></p>
              </p>
            </div>
          </div>
        </div>
